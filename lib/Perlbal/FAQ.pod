...
It's currently a bit short, but exists nonetheless as a place where other can contribute to share their knowledge.


=head2 General questions on Perlbal

=head3 Does Perlbal have documentation?
...
It knows all about persistent connections (in both 1.0 and 1.1) and will reply with HTTP/1.0 Connection: keep-alive the request was only implicitly keep-alive with HTTP/1.1.  etc, etc.
...
Yes. To use SSL mode you'll need L<IO::Socket::SSL> C<v0.97+>.

You can do SSL either on C<web_server>, C<reverse_proxy> or C<selector> modes, but not if the C<service> selector is vhost-based, because SSL and vhosts aren't compatible.
