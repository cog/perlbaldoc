...
Perl has a high priority queue for sending requests through to backends quickly, using cookies to determine if a request should go to fast queue (configurable).
