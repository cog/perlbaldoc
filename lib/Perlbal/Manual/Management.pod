...
=item * pool <name> add|remove <ip[:port]>
...
=item * show service|pool [<service_name|pool_name>]
...
Shows the time the server was started and the current uptime.
