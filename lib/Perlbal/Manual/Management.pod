...
Using C<BSD::Resource>, sets the maximum number of open files for this process.

Requires C<BSD::Resource>. Also, Perlbal must be run under a superuser.
...
One of C<none>, C<linux> (for C<Linux::AIO>), or C<ioaio> (for C<IO::AIO>).

This controls how disk IO is done asynchronously. Highly recommended to use C<Linux::AIO> or C<IO::AIO> for webserving or reproxying files. For purely reverse proxy or only reproxying URLs, none is fine.
...
If a service uses a plugin that supports C<dumpconfig>, that plugin's configuration is also dumped (see C<dumpconfig> under C<Perlbal::Manual::Plugins> for more information).
...
The environment variable C<DEBUG_OBJ> must be set to a true value (see C<Environment Variables> under C<Perlbal::Manual::Configuration> for more information).
