 ...
 * max_connections
 * nice_level
 * aio_mode - one of "none", "linux" (Linux::AIO), or "ioaio" (IO::AIO). This controls how disk IO is done asynchronously. Highly recommended to use C<Linux::AIO> or C<IO::AIO> for webserving or reproxying files. For purely reverse proxy or only reproxying URLs, none is fine.
 * aio_threads - number of child threads doing disk IO. Use between 2 and 50.
