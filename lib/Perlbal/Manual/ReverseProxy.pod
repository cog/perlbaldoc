=head1 Using Perlbal as a Reverse Proxy
...
This document details reverse proxy configuration of Perlbal 1.76.
...
Check C<Perlbal::Manual::LoadBalancer> under C<Configuring Perlbal as a Load Balancer> for a sample configuration file and refer to the section C<Load Balancer vs. Reverse Proxy> on that file for an explanation of the differences between a Load Balancer and a Reverse Proxy.
...
The connection is killed, as we already have two other open connections (the first and the third), and that's the number set by Cbackend_persist_cache> for the number of connections to be kept alive.
