...
If Perlbal is running on the foreground, it logs by calling C<printf>, which means you should get the log on C<STDOUT>.
...
You should see the documentation for L<Sys::Syslog> for more information, but here's an example:
...
It's similar to connecting to a management service and issue a C<show service> for each service, plus a Cstates> and a C<queues> command.
...
You can tweek L<Sys::Syslog>'s configuration under F</etc/syslog.conf>. See L<Sys::Syslog>.
