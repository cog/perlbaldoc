...
The C<plugins> parameter (a list of strings separated by commas or spaces) defined which plugins are acceptable for that service.
...
A Perlbal plugin consists in a package under the C<Perlbal::Plugin> namespace that implements a set of functions described below.
...
That should create a file tree that you can get better acquainted to by reading L<Module::Starter>'s fine documentation. For this example, the file you really need should be C<lib/Perlbal/Plugin/ColorOfMagic.pm>.
...
Your selector function receives a L<Perlbal::ClientHTTPBase> object.
...
L<Perlbal::Manual::Hooks>, L<Perlbal::Manual::Internals>.
