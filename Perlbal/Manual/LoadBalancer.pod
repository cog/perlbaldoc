...
Whenever Perlbal fails to open a connection to a machine, it assumes that machine is not available and spreads the load between the remaining servers.

... mention C<idle_timeout> ...
... mention C<error_retry_schedule> ...
...
...
A reverse proxy can cache static content from your servers and serve it directly in order to reduce the load on your machines

=item security

Think of a reverse proxy as something that stands in the way between the users and your servers; this reverse proxy can add an extra layer of protection to your servers
